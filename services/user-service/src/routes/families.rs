...
pub fn router() -> Router<AppState> {
    Router::new()
        ...
        .route(
            "/families/:id",
            get(get_family).patch(update_family).delete(dissolve_family),
        )
        .route("/families/:id/members", get(list_members))
        .route(
            "/families/:id/members/:uid",
...
fn to_mdto(m: MemberRow) -> MemberDto {
    MemberDto {
        ...
        role: m.role,
        nickname: m.nickname,
        joined_at: m.joined_at.assume_utc().to_string(),
    }
}
fn gen_code() -> String {
    use rand::Rng;
    ...
    (0..6)
        .map(|_| {
            let i = r.random_range(0..36usize);
            if i < 10 {
                (b'0' + i as u8) as char
            } else {
...
async fn create_family(
    ...
    let ex: Option<i32> =
        sqlx::query_scalar("SELECT family_id FROM family_members WHERE user_id=$1 LIMIT 1")
            .bind(uid)
            .fetch_optional(&s.db)
            .await
            .unwrap_or(None);
    ...
    sqlx::query(
        "INSERT INTO family_members (family_id,user_id,role,nickname) VALUES ($1,$2,'owner',$3)",
    )
    .bind(fam.id)
    .bind(uid)
    .bind(&req.nickname)
    ...
    Ok((
        ...
        axum::Json(
            serde_json::to_value(ok(
                serde_json::json!({"family": to_fdto(fam, 1)}),
                "创建成功",
                rid,
            ))
            .unwrap(),
...
async fn my_family(State(s): State<AppState>, h: HeaderMap) -> Res {
    ...
    Ok(axum::Json(
        serde_json::to_value(ok(
            serde_json::json!({"family": to_fdto(fam, c)}),
            "ok",
            rid,
        ))
        .unwrap(),
...
async fn update_family(
    ...
    let role = get_role(&s.db, id, uid).await.ok_or_else(|| {
        err(
            StatusCode::FORBIDDEN,
            "FORBIDDEN",
            "无权操作",
...
async fn join_family(
    ...
    let fam: Option<FamilyRow> = sqlx::query_as("SELECT * FROM families WHERE invite_code=$1")
        .bind(req.invite_code.to_uppercase())
        .fetch_optional(&s.db)
        .await
        .map_err(|_| {
            err(
    ...
    sqlx::query(
        "INSERT INTO family_members (family_id,user_id,role,nickname) VALUES ($1,$2,'member',$3)",
    )
    .bind(fam.id)
    .bind(uid)
    .bind(&req.nickname)
...
async fn update_member(
    ...
    let role = get_role(&s.db, fid, uid).await.ok_or_else(|| {
        err(
            StatusCode::FORBIDDEN,
            "FORBIDDEN",
            "无权操作",
    ...
    .map_err(|_| err(StatusCode::INTERNAL_SERVER_ERROR, "INTERNAL_ERROR", "更新失败", None, rid.clone()))?;
    Ok(axum::Json(
        serde_json::to_value(ok(serde_json::json!({}), "更新成功", rid)).unwrap(),
    ))
}
...
async fn remove_member(
    ...
    Ok(axum::Json(
        serde_json::to_value(ok(serde_json::json!({}), "移除成功", rid)).unwrap(),
    ))
}
...
async fn leave_family(State(s): State<AppState>, h: HeaderMap, Path(id): Path<i32>) -> Res {
    ...
    Ok(axum::Json(
        serde_json::to_value(ok(serde_json::json!({}), "已退出家庭", rid)).unwrap(),
    ))
}
...
async fn dissolve_family(State(s): State<AppState>, h: HeaderMap, Path(id): Path<i32>) -> Res {
    ...
    Ok(axum::Json(
        serde_json::to_value(ok(serde_json::json!({}), "家庭已解散", rid)).unwrap(),
    ))
}
